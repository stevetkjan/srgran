...
#    for i in range(len(valid_hr_imgs)):
    for i in range(batch_size):
        tl.vis.save_image(scipy.misc.imresize(valid_hr_imgs[0], [xx,xx]), save_dir + '/valid_lr_in.png')
        valid_hr_imgs1.append(scipy.misc.imresize(valid_hr_imgs[0], [xx,xx]))
#        valid_hr_imgs1.append(valid_hr_imgs[0])
#        valid_hr_img = valid_hr_imgs[i]
 #       valid_lr_img = (valid_lr_img / 127.5) - 1  # rescale to ［－1, 1]
#        transformed_images.append(scipy.misc.imresize(valid_lr_img, [339, 510]))
  #      transformed_images.append(resize(valid_lr_img,( 339,510)))


    #test_x = np.stack(transformed_images)
    size = valid_lr_img.shape
    #print('================================================')
    #print(valid_lr_img.shape,test_x.shape)
    ...
    b_imgs_96 = tl.prepro.threading_data(valid_hr_imgs1[0: batch_size],fn=pre)
...
#    out = sess.run(net_g.outputs, {t_image: [valid_lr_img]})
    for x in range(0, 1000, batch_size):
        ...
        start_time = time.time()
        out = sess.run(net_g.outputs, {t_image: b_imgs_96})
        step_time = time.time()
    ...
    for idx in range(0, 5000, batch_size):
        ...
        #print('=x=x=x=x=x==x=x=x=x'+str(len(valid_hr_imgs))+str(batch_size))
        #for i in range(batch_size):
        #    tl.vis.save_image(out[i], save_dir + '/valid_gen'+str(idx+i)+'.png')
            ## update D
            #errD, _ = sess .run([d_loss, d_optim], {t_image: b_imgs_96, t_target_image: b_imgs_384})
            ## update G
            #errG, errM, errV, errA, _ = sess.run([g_loss, mse_loss, vgg_loss, g_gan_loss, g_optim], {t_image: b_imgs_96, t_target_image: b_imgs_384})
    print(batch_size,len(total_time),(len(total_time)*batch_size) )
    tl.vis.save_image(out[0], save_dir + '/valid_output.png')
    print(sum(total_time))
    print('%d %4.2f' %(   batch_size,1/(sum(total_time)/(len(total_time)*batch_size) ))   )
    #print(sum(total_time)/(len(total_time)*batch_size) )
    #print(1/(sum(total_time)/(len(total_time)*batch_size) ))
    #print(sum(total_time)/(len(total_time)*batch_size) )
    #print(1/(sum(total_time)/(len(total_time)*batch_size) ))
    ...
    """
    for x in range(num_iter):
        for idx in range(0, len(valid_lr_imgs), bs):
   
            transformed_images = []
            for i in range(bs):
            

               # print((idx+i),len(valid_lr_imgs))
                valid_lr_img = valid_lr_imgs[idx+i]
                valid_lr_img = (valid_lr_img / 127.5) - 1  # rescale to ［－1, 1]
            #  transformed_images.append(resize(valid_lr_img,( 339,510)))
...
"""
    ...
    #print("[*] save images")
   # tl.vis.save_image(out[0], save_dir + '/valid_gen1.png')
    #tl.vis.save_image(out[1], save_dir + '/valid_gen2.png')
    #for i in range(len(valid_hr_imgs)):
   # for i in range(bs):
    #    tl.vis.save_image(out[i], save_dir + '/valid_gen'+str(i)+'.png')
#        valid_hr_img = valid_hr_imgs[i]
